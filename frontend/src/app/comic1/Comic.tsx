...
const Comic: React.FC = () => {
  ...
  useEffect(() => {
    const runCombinedWorkflow = async () => {
      ...
      try {
        const res = await fetch("https://backend.diffrun.com/run-combined-workflow", {
          method: "POST",
      ...
      try {
        const pollForImage = async (retries = 10) => {
          for (let i = 0; i < retries; i++) {
            const response = await fetch(`https://backend.diffrun.com/get-combined-image?job_id=${jobId}`);
            if (response.ok) {
  ...
  const handleGeneratePdf = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://backend.diffrun.com/generate-comic-pdf?job_id=${jobId}`);
