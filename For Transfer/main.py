...
def approve_for_printing(
    ...
    try:
        ...
        exterior_index = selected[0]  # This is the cover image index
        ...
        return {
            "status": "success",
            "message": "Step 1–5 complete: interior PDF uploaded & cover workflow started"
        }
...
def upload_approved_images_to_s3(job_id: str, local_folder: str, selected_indices: List[int], book_id: Optional[str] = None):
    interior_dir = os.path.join(local_folder, "interior")
    exterior_dir = os.path.join(local_folder, "exterior")

    os.makedirs(interior_dir, exist_ok=True)
    os.makedirs(exterior_dir, exist_ok=True)

    for page_num, variant_index in enumerate(selected_indices):
        if variant_index is None:
            logger.warning(f"⚠️ Skipping page {page_num}: no selected variant.")
            continue

        variant_num = variant_index + 1

        # ✅ Use alternate filename logic for exterior (page 0)
        if page_num == 0:
            if not book_id:
                logger.warning(f"⚠️ book_id is required to construct exterior filename for page 0.")
                continue
            filename = f"_{job_id}_{book_id}_{variant_num:05d}_.png"
            src_path = os.path.join(exterior_dir, filename)
            dest_path = src_path
            s3_folder = f"approved_output/{job_id}/exterior"
        else:
            filename = f"_{job_id}_{page_num:02d}_{variant_num:05d}_.png"
            src_path = os.path.join(interior_dir, filename)
            dest_path = src_path  # already inside interior/
            s3_folder = f"approved_output/{job_id}/interior"

        if not os.path.exists(src_path):
            logger.warning(f"❌ Image not found for page {page_num}: {filename}")
            continue

        if page_num == 0:
            if os.path.abspath(src_path) != os.path.abspath(dest_path):
                shutil.copy(src_path, dest_path)
                logger.info(f"✅ Copied approved exterior image: {filename}")
            else:
                logger.info(f"🔁 Skipped copy: identical source and destination for exterior")

        try:
            s3.upload_file(dest_path, "diffrungenerations", f"{s3_folder}/{filename}")
            logger.info(f"📤 Uploaded to S3: {s3_folder}/{filename}")
        except Exception as e:
    ...
    for file in os.listdir(exterior_path):
        if file.endswith(".png"):
            ...
            try:
                ...
                logger.info(
                    f"✅ Uploaded regenerated pg0 cover to s3://diffrungenerations/{s3_key}")
            except Exception as e:
...
def generate_pg0_covers_for_stories(
    ...
    base_dir = "D:/ComfyUI_windows_portable/ComfyUI/input/stories"
    name = name.capitalize()
    gender = gender.lower()

    logger.info(
        f"🧢 Generating cover pages (pg0) for all stories for job_id={job_id}")

    if not os.path.exists(base_dir):
        raise HTTPException(
            status_code=500, detail="Stories base folder not found")
    ...
    if exclude_book_id:
        story_books = [b for b in story_books if b.lower() !=
                       exclude_book_id.lower()]
        logger.info(
            f"📦 Excluding book: {exclude_book_id}, Remaining: {story_books}")
    ...
    if not saved_filenames:
        raise HTTPException(
            status_code=400, detail="No input images found for this job.")
    ...
    for book_id in story_books:
        ...
        if not os.path.exists(pg0_folder):
            logger.warning(
                f"⏭️ Skipping {book_id} (pg0 folder missing for {gender})")
            continue
        ...
        final_pg0_path = os.path.join(
            "D:/ComfyUI_windows_portable/ComfyUI/output", job_id)
